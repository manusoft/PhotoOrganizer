...
public class PhotoViewModelBuilder
{
    ...
    {
        PhotoViewModel photoViewModel = new PhotoViewModel(_file, _thumbnailService);
        ...
        if(_outputBaseFolderPath is not null && _outputFolderFormat is not null && photoViewModel.DateTaken is not null)
        {
            photoViewModel.OutputFilePath = CreateDateTimeFormatedFolderPath(photoViewModel.DateTaken, _outputBaseFolderPath, _outputFolderFormat);
        }

        return photoViewModel;
    }

    private string? CreateDateTimeFormatedFolderPath(DateTime? dateTaken, string outputBaseFolderPath, string outputBaseFolderFormat)
    {
        StringBuilder stringBuilder = new();

        return stringBuilder
            .Append(outputBaseFolderPath)
            .Append(dateTaken?.ToString(outputBaseFolderFormat))
            .ToString();
    }
